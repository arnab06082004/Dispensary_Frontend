  ...
  const header = ["Sl.no", "Name", "Quantity", "Usage"];
   ...
   const formattedData = (data) => {
      const newArr = data.map((item, idx) => {
        return {sl : idx + 1, name : item.name, quantity : item.quantity, usage : item.usage }
   }) 
   setRowData(newArr)
   }
   ...
   useEffect(() => {
    const fetchData = async() => {
      ...
      axios.get(`http://localhost:4000/api/medicine/search-by-name?name=${medicineName}`).then((response) => {
        
        if(response.data.medicine.length === 0)
          setRowData([])
